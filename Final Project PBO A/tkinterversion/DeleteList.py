...
class DeleteList(tk.Tk, TodoList):
    ...
    def __init__(self):
        ...
        self.label_3.grid(row=2, pady=5)

        self.label_4 = Label(
            self.frame_1,
            text="Masukkan nomor kegiatan yang ingin dihapus :",
        ...
        self.entry_delete.grid(row=4, pady=5)
        ...
        self.delete_button = Button(
            ...
            borderwidth=0,
            relief=RIDGE,
            command=lambda: self.deleteList(self.onClickAction(), self),
        ).grid(row=5, pady=10)
        # mengatur judul, background, editable, dan menambahkan shortcut esc untuk exit program
        self.__centerWindow(890, 410)
        self.title("Delete List")
    ...
    def onClickAction(self):
        try:
            return int(self.entry_delete.get())
        except ValueError:
